                    ...
                    <div class="col-sm-6">
                        @if(auth()->user()->can('vouchers create'))
                            <a class="btn btn-primary float-end" data-bs-toggle="modal"
                               wire:click.prevent="CreateVoucher"
                               data-bs-target="#CreateVoucher" data-bs-original-title=""
                               title=""> {{__('Create Voucher')}}</a>
                        @endif
                    </div>
                    <div class="col-sm-6">
                        @if(auth()->user()->can('vouchers pay pay'))
                            <a class="btn btn-primary float-end" data-bs-toggle="modal"
                               wire:click.prevent="PayVoucher"  data-bs-target="#PayVoucher" data-bs-original-title=""
                               title=""> {{__('Pay Voucher')}}</a>
                        @endif
                    </div>
                </div>
            </div>
                                    ...
                                    <table class="table color-bordered-table info-table  info-bordered-table">
                                        ...
                                        @foreach($vouchers as $key => $voucher)
                                            <tr>
                                                ...
                                                <td>{{$voucher->code}}</td>
                                                <td>{{$voucher->symbol . $voucher->price}}</td>
                                                <td>{{$voucher->user ? $voucher->user->name : ''}}</td>
...
@if(auth()->user()->can('vouchers pay pay'))
    <!--  Modal PayVoucher -->
        <div wire:ignore.self class="modal fade " id="PayVoucher" tabindex="-1" role="dialog"
             aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content text-center">
                    <div class="modal-header text-center">
                        <h5 class="modal-title text-center" id="exampleModalLongTitle">{{ __('Pay Vouchers') }}</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true close-btn"></span>
                        </button>
                    </div>
                    <div class="modal-body">
                        ...
                        @if($pay_voucher)
                            @livewire('admin.vouchers.vouchers-pay')
                        @endif
                    </div>
                </div>
            </div>
